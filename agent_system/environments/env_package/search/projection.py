...
def think_prefix_projection(actions: List[str]) -> Tuple[List[str], List[int]]:
    """Project a list of LLM *actions* into (`results`, `valids`).

    Extraction logic:
        1. Grab everything before the **first** closing ``</think>`` tag
           (case-insensitive).
        2. Do not keep the closing tag itself.
        3. If no closing ``</think>`` tag exists, store an empty string.

    Validity logic:
        - ``valids[i]`` is 1 iff the original action contains at least one
          closing ``</think>`` tag (case-insensitive).
        - Otherwise ``valids[i]`` is 0.
    """
    ...
    # --- Pre-compiled patterns -----------------------------------------------
    re_think_prefix = re.compile(r"^(.*?)</think>", re.IGNORECASE | re.DOTALL)
    ...
    for i, action in enumerate(actions):
        ...
        # --- Extraction -------------------------------------------------------
        m = re_think_prefix.search(action)
        if m:
        ...
        # --- Validity checks --------------------------------------------------
        n_think_close = len(re_think_close_tag.findall(original_action))
        if n_think_close == 0:
            valids[i] = 0
