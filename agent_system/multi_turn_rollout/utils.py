...
def adjust_batch(config, data: DataProto, mode="copy", info_gain_compute=False) -> DataProto:
    ...
    # to compute info gain, we need to align batch size only to rollout
    if info_gain_compute:
        size_divisor_actor = size_divisor_rollout = size_divisor_ref = config.actor_rollout_ref.rollout.info_gain_compute_log_prob_micro_batch_size_per_gpu * world_size
    ...
    if remainder == 0:
        return balance_batch(data, world_size) if info_gain_compute else data
    ...
    return balance_batch(adjusted_batch, world_size) if info_gain_compute else adjusted_batch

def balance_batch(batch: DataProto, world_size: int):
