...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    tree_structure = 'traj_uid' not in batch.non_tensor_batch.keys() and 'node_uid' in batch.non_tensor_batch.keys()
    if tree_structure:
        # orgin means the node is the First step(its tree depth = 1)
        origin_index = np.where(batch.non_tensor_batch['traj_step'] == 0)[0]
        origin_num = len(origin_index) # equals to GRPO traj num
        origin_rewards = batch.non_tensor_batch['rewards'][origin_index]
        origin_episode_lengths = batch.non_tensor_batch['subtree_traj_depths'][origin_index]
        origin_tool_callings = batch.non_tensor_batch['subtree_tool_callings'][origin_index]
    else:
    ...
    metrics = {
        ...
        **(
            ...
            else 
            {
                # episode
                "episode/reward/mean": 
                    origin_rewards.mean().item(),
                "episode/reward/max": 
                    origin_rewards.max().item(),
                "episode/reward/min": 
                    origin_rewards.min().item(),
                "episode/length/mean": 
                    origin_episode_lengths.mean().item(),
                "episode/length/max":
                    origin_episode_lengths.max().item(),
                "episode/length/min": 
                    origin_episode_lengths.min().item(),
                "episode/tool_call_count/mean": 
                    origin_tool_callings.mean().item(),
            }
        ),
